...
The pipeline follows a structured deterministic + AI decision model:

### 1. Alert filtering
- Alerts are pulled from Wazuh at scheduled intervals
- Only alerts with rule level ≥ 3 are processed
- Lower-level noise is ignored

### 2. Severity-based routing
- If severity ≥ 5:
  - Alert bypasses the LLM
  - Immediately sent to Discord as a critical incident

- If severity < 5:
  - Alert is sent to Llama 3.2 via Ollama
  - The model evaluates the event and returns:
    - true positive or false positive classification
    - confidence score
    - reasoning
    - recommended action
...
### 3. Final output
- Enriched alert is generated by combining:
  - original telemetry
  - severity evaluation
  - LLM classification output
- Final structured incident is sent to Discord
